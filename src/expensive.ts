...
export function sharedSelector(items: Item[], version: number, predicateKey: number): Derived {
  if (cacheVal && cacheKey.version === version && cacheKey.predicateKey === predicateKey) {
    return cacheVal
  }
